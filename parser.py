...
def apply(type, trans, points, screen, color):
    matrix_mult(trans, points)
    draw_edges(points, screen, color) if type == "edges" else draw_polygons(points,screen,color)
    points = []

def alter(trans,stack):
    matrix_mult(stack[0], trans)
    stack[0] = trans
...
def parse_file( fname, edges, polygons, transform, screen, color ):
    ...
    while c < len(lines):
        ...
        if line == 'sphere':
            ...
            apply("polygons", transform[0], polygons, screen, color)
            polygons=[]
            ...
            apply("poygons", transform[0], polygons, screen, color)
            polygons=[]

        elif line == 'push':
            transform.insert(0, transform[0][:])
            ...
            add_circle(edges,
                       float(args[0]), float(args[1]), float(args[2]),
                       float(args[3]), step)
            apply("edges", transform[0], edges, screen, color)
            edges = []
